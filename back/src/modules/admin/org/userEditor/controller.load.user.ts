/**
 * controller.load.user.ts - backend file
 * version: 1.0.02
 * 
 * Controller for loading user data.
 * 
 * Handles HTTP requests to the /api/admin/users/load/:userId endpoint.
 * Validates JWT (handled by middleware), retrieves user data from the service,
 * and sends the response. Uses event bus to track operations.
 * 
 * Note: HTTP request/response events are now handled by the universal connection handler.
 */

import { Request, Response } from 'express';
import { loadUserById as loadUser } from './service.load.user';
import { connectionHandler } from '../../../../core/helpers/connection.handler';

/**
 * Business logic for loading user data
 * 
 * @param req - Express Request object
 * @param res - Express Response object
 */
async function loadUserLogic(req: Request, res: Response): Promise<any> {
    // JWT validation is already performed by route guards
    // If request reaches controller, JWT is valid

    // Get user ID from URL parameters
    const userId = req.params.userId;

    // Load user data
    const result = await loadUser(userId, req);

    return result;
}

// Export controller using universal connection handler
export default connectionHandler(loadUserLogic, 'LoadUserController');