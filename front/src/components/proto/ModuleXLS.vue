<template>
  <div class="pt-3 pl-3">
    <v-card max-width="600px" color="grey lighten-4">
      <v-card-title class="text-h5">модуль для прототипирования функций excel</v-card-title>
      <v-card-text>
      <form @submit.prevent="handleSubmit">
        <div>
          <label for="textField">Название организации:</label>
          <input type="text" id="orgname" v-model="formData.orgname" />
        </div>
        <div>
          <label for="regionSelect">Страна / Регион:</label>
          <select id="regionSelect" v-model="formData.region">
        ...
        <div>
          ...
          <select id="locationSelect" v-model="formData.location">
        ...
        <div>
          <input type="radio" id="radio1" value="Option 1" v-model="formData.radio" />
          <label for="radio1">опция 1</label>
          <input type="radio" id="radio2" value="Option 2" v-model="formData.radio" />
          <label for="radio2">опция 2</label>
        </div>
        <div>
          <label for="datePicker">Дата заполнения:</label>
          <Datepicker id="datePicker" v-model="formData.date" format="yyyy-MM-dd"></Datepicker>
        </div>
        <br>
        <br>
        <br>
        <v-btn color="teal darken-2" type="button" @click="generateExcelFile">Сгенерировать файл Excel</v-btn><br><br>
        <v-btn color="teal" type="submit">записать в бд</v-btn><br><br>
        <v-btn color="teal" density="compact" elevation="20"> компактная кнопка</v-btn><br>
