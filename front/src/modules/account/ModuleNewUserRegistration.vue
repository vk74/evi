      ...
      <v-card-text>
        <v-form @submit.prevent="submitForm">
          ...
          <span
            ...
            class="user-registration-form-info"
          >Имя пользователя должно быть не более 25 символов и содержать только буквы и цифры.</span>
          ...
          <span
            ...
            class="user-registration-form-info"
          >Пароль должен быть от 8 до 40 символов и содержать только буквы, цифры и знаки препинания.</span>
          ...
          />
          <span
            v-if="isInvalid('surname')"
            class="user-registration-form-info"
          >Фамилия должна быть не более 25 символов и содержать только буквы.</span>
          ...
          <span
            v-if="isInvalid('name')"
            class="user-registration-form-info"
          >Имя должно быть не более 25 символов и содержать только буквы.</span>
          ...
          <span
            ...
            class="user-registration-form-info"
          >Телефон должен содержать только цифры и быть не длиннее 12 символов.</span>
          ...
          <span
            ...
            class="user-registration-form-info"
          >Адрес должен быть не более 100 символов и содержать только буквы, цифры, пробелы и знаки препинания.</span>
        </v-form>
        <p
          v-if="showError"
          class="error-message"
        >
          Ошибка валидации: невалидные поля - {{ invalidFields.join(', ') }}
        </p>
        <p
          v-if="showDuplicateUsernameError"
          class="error-message"
        >
          ошибка: такое имя пользователя уже зарегистрировано
        </p>
        <p
          v-if="showDuplicateEmailError"
          class="error-message"
        >
          ошибка: такой адрес электронной почты уже используется
        </p>
        <p
          v-if="showDuplicatePhoneError"
          class="error-message"
        >
          ошибка: такой номер телефона уже используется
        </p>
        <p
          v-if="showSuccess"
          class="success-message"
        >
          Данные регистрационной формы успешно отправлены на сервер
        </p>
      </v-card-text>
      <v-card-actions>
        <v-spacer />
