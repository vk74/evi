<!--
  File: Application.Work.vue - frontend file
  Description: Work module visibility settings
  Purpose: Enable/disable work module visibility
  Version: 1.1.0
  
  Features:
  - Work module visibility enable/disable
...
const section_path = 'Application.Work';
...
function updateLocalSetting(settingName: string, value: any) {
  switch (settingName) {
    case 'work.module.is.visible':
      workModuleVisible.value = Boolean(value);
      break;
  }
}
...
async function loadSettings() {
  ...
  try {
    console.log('Loading settings for Application.Work');
...
watch(
  ...
  (newValue) => {
    if (!isFirstLoad.value) {
      updateSetting('work.module.is.visible', newValue);
    }
  }
);
...
onMounted(() => {
  console.log('Application.Work component initialized');
  loadSettings();
});
</script>

<template>
  <div class="work-module-container">
    <h2 class="text-h6 mb-4">
      {{ t('admin.settings.application.work.title') }}
    </h2>
