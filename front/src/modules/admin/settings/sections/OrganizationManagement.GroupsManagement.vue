<!--
  File: OrganizationManagement.GroupsManagement.vue
  Version: 1.4.0
...
const section_path = 'OrganizationManagement.GroupsManagement';
...
async function loadSetting(settingName: string): Promise<boolean> {
  ...
  try {
    ...
    if (settingRetryAttempts.value[settingName] < 1) {
      ...
      uiStore.showErrorSnackbar(`${t('admin.settings.organizationmanagement.groupsmanagement.messages.error.loading')}: ${settingName}`);
    }
...
async function loadSettings() {
  ...
  try {
    ...
    if (settings && settings.length > 0) {
      ...
      uiStore.showSuccessSnackbar(t('admin.settings.organizationmanagement.groupsmanagement.messages.settings.loaded'));
    } else {
    ...
    console.error('Failed to load groups management settings:', error);
...
onMounted(() => {
  console.log('OrganizationManagement.GroupsManagement component initialized');
  loadSettings();
});
</script>
...
<template>
  <div class="groups-management-container">
    <h2 class="text-h6 mb-4">
      {{ t('admin.settings.organizationmanagement.groupsmanagement.title') }}
    </h2>
      ...
      <div class="section-content">
        <div class="d-flex align-center mb-2">
          <v-switch
            ...
            color="teal-darken-2"
            :label="t('admin.settings.organizationmanagement.groupsmanagement.only.active.members.label')"
            hide-details
          ...
          >
            ...
            <div class="pa-2">
              <p class="text-subtitle-2 mb-2">
                {{ t('admin.settings.organizationmanagement.groupsmanagement.messages.error.tooltip.title') }}
              </p>
              <p class="text-caption">
                {{ t('admin.settings.organizationmanagement.groupsmanagement.messages.error.tooltip.retry') }}
              </p>
            </div>
          </v-tooltip>
        </div>
