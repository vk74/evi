<!--
  ...
  Changes in v1.1.0:
  - Removed wrapper component CatalogSectionEditor.vue
  - Integrated navigation bar directly into component
  - Added change tracking with initialSectionData ref
  - Added hasChanges computed property to track form modifications
  - SAVE button only enabled when changes detected
  - Added glow effect animation for active SAVE button
  - Updated service import path to local directory
...
// Initial section data for change tracking
const initialSectionData = ref<{
  name: string
  icon_name: string
  owner: string
  backupOwner: string
...
const hasChanges = computed(() => {
  if (!isEditMode.value || !initialSectionData.value) {
    return false
  }
  
  const initial = initialSectionData.value
...
const populateFormWithSection = (section: CatalogSection) => {
  ...
  formData.value = sectionData
  
  // Store initial data for change tracking
  initialSectionData.value = { ...sectionData }
...
const updateSection = async () => {
  ...
  if (!hasChanges.value) {
    uiStore.showErrorSnackbar(t('admin.catalog.editor.messages.update.noChanges') || 'Нет изменений для сохранения')
    return
  }
  ...
  try {
    ...
    // Update initial data after successful update to reset change tracking
    if (initialSectionData.value) {
      initialSectionData.value = {
        name: sectionData.name || '',
        icon_name: sectionData.icon_name || '',
        owner: sectionData.owner || '',
        backupOwner: sectionData.backup_owner || '',
        order: sectionData.order || 1,
        status: sectionData.status || 'draft',
        isPublic: sectionData.is_public || false,
        ...
        <div class="side-bar-container">
          <div class="side-bar-section">
            ...
            <v-btn
              ...
              :class="['mb-3', { 'btn-glow-active': isFormValid && !isSubmitting }]"
              @click="createSection"
            >
