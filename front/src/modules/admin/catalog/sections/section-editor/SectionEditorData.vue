  ...
  <div class="d-flex">
    <div class="flex-grow-1">
      <v-container class="content-container">
        <DataLoading v-if="isLoadingSection" :loading="isLoadingSection" loading-text="Загрузка данных секции..." size="medium" overlay />
        <v-card flat>
          <v-form ref="form" v-model="isFormValid">
            <v-row>
              <v-col cols="12">
                <div class="card-header">
                  <v-card-title class="text-subtitle-1">{{ t('admin.catalog.editor.information.title') }}</v-card-title>
                  <v-divider class="section-divider" />
                </div>
                <v-row class="pt-3">
                  <v-col cols="12" md="auto" style="width: 80px;">
                    <div class="icon-placeholder" @click="openIconPicker" style="cursor: pointer;">
                      <component v-if="selectedIconComponent" :is="selectedIconComponent" :size="24" color="rgb(20, 184, 166)" class="placeholder-icon" />
                      <div v-else class="empty-placeholder">
                        <v-icon size="24" color="rgb(20, 184, 166)">mdi-image-outline</v-icon>
                      </div>
                    </div>
                  </v-col>
                  <v-col cols="12" md="auto" style="width: 200px;">
                    <v-text-field v-model="formData.order" :label="t('admin.catalog.editor.information.order.label')" variant="outlined" density="comfortable" type="number" min="1" :rules="orderRules" color="teal" />
                  </v-col>
                  <v-col cols="12" md="auto" style="flex-grow: 1;">
                    <v-text-field v-model="formData.name" :label="t('admin.catalog.editor.information.name.label')" :rules="nameRules" variant="outlined" density="comfortable" counter="100" required color="teal" />
                  </v-col>
                </v-row>
                ...
                <v-row>
                  <v-col cols="12" md="6">
                    <div class="d-flex align-center">
                      <v-text-field v-model="formData.owner" :label="t('admin.catalog.editor.information.owner.label')" :rules="ownerRules" readonly append-inner-icon="mdi-account-search" @click:append-inner="showOwnerSelector = true" required color="teal" />
                    </div>
                  </v-col>
                  <v-col cols="12" md="6">
                    <div class="d-flex align-center">
                      <v-text-field v-model="formData.backupOwner" :label="t('admin.catalog.editor.information.backupOwner.label')" readonly append-inner-icon="mdi-account-search" @click:append-inner="showBackupOwnerSelector = true" color="teal" />
                    </div>
                  </v-col>
                </v-row>
              </v-col>
              ...
              <v-col cols="12">
                <div class="card-header mt-6">
                  <v-card-title class="text-subtitle-1">{{ t('admin.catalog.editor.settings.title') }}</v-card-title>
                  <v-divider class="section-divider" />
                </div>
                <v-row class="pt-3">
                  <v-col cols="12" md="4">
                    <v-select v-model="formData.status" :label="t('admin.catalog.editor.settings.status.label')" variant="outlined" density="comfortable" :items="statusOptions" item-title="title" item-value="value" color="teal" />
                  </v-col>
                  <v-col cols="12" md="4">
                    <v-checkbox v-model="formData.isPublic" :label="t('admin.catalog.editor.settings.isPublic.label')" variant="outlined" density="comfortable" color="teal" />
                  </v-col>
                  <v-col cols="12" md="4">
                    <div class="color-picker-container">
                      <v-text-field v-model="formData.color" :label="t('admin.catalog.editor.settings.color.label')" variant="outlined" density="comfortable" placeholder="#1976D2" :rules="[v => /^#[0-9A-Fa-f]{6}$/.test(v) || t('admin.catalog.editor.settings.color.picker.validHex')]" color="teal">
                        <template #prepend-inner>
                          <div class="color-preview" :style="{ backgroundColor: formData.color }" @click="showColorPicker = true" />
                        </template>
                        <template #append-inner>
                          <v-btn icon variant="text" size="small" @click="showColorPicker = true"><v-icon>mdi-palette</v-icon></v-btn>
                        </template>
                      </v-text-field>
                      <v-dialog v-model="showColorPicker" max-width="600">
                        <v-card>
                          <v-card-title class="text-subtitle-1">{{ t('admin.catalog.editor.settings.color.picker.title') }}</v-card-title>
                          <v-card-text>
                            <div class="color-picker-content">
                              <div class="color-preview-large mb-4">
                                <div class="preview-box" :style="{ backgroundColor: selectedColor }" />
                              ...
                              <div class="preset-colors mb-4">
                                ...
                                <div class="color-grid">
                                  <div v-for="color in presetColors" :key="color" class="color-swatch" :style="{ backgroundColor: color }" @click="selectedColor = color" />
                                </div>
                              </div>
                              <div class="custom-color-input">
                                <div class="input-title mb-2">{{ t('admin.catalog.editor.settings.color.picker.customColor') }}</div>
                                <v-text-field v-model="selectedColor" :label="t('admin.catalog.editor.settings.color.picker.hexCode')" variant="outlined" density="compact" placeholder="#000000" :rules="[v => /^#[0-9A-Fa-f]{6}$/.test(v) || t('admin.catalog.editor.settings.color.picker.validHex')]" color="teal" />
                              </div>
                            </div>
                          </v-card-text>
                          <v-card-actions>
                            <v-spacer />
                            <v-btn color="grey" variant="text" @click="showColorPicker = false">{{ t('admin.catalog.editor.actions.cancel') }}</v-btn>
                            <v-btn color="teal" variant="text" @click="applyColor">{{ t('admin.catalog.editor.actions.save') }}</v-btn>
                          </v-card-actions>
                        </v-card>
                      </v-dialog>
                    </div>
              ...
              <v-col cols="12">
                <div class="card-header mt-6">
                  <v-card-title class="text-subtitle-1">{{ t('admin.catalog.editor.description.title') }}</v-card-title>
                  <v-divider class="section-divider" />
                </div>
                <v-row class="pt-3">
                  <v-col cols="12">
                    <v-textarea v-model="formData.description" :label="t('admin.catalog.editor.description.section.label')" :rules="descriptionRules" variant="outlined" rows="3" counter="1000" no-resize color="teal" />
                  </v-col>
                </v-row>
                <v-row>
                  <v-col cols="12">
                    <v-textarea v-model="formData.comments" :label="t('admin.catalog.editor.description.comments.label')" :rules="commentsRules" variant="outlined" rows="3" counter="500" no-resize color="teal" />
                  </v-col>
                </v-row>
              </v-col>
            </v-row>
    ...
    <div class="side-bar-container">
      <div class="side-bar-section">
        <h3 class="text-subtitle-2 px-2 py-2">{{ t('admin.catalog.editor.actions.title') }}</h3>
        <div class="icon-picker-sidebar mb-3">
          <v-btn block variant="outlined" color="teal" prepend-icon="mdi-image-outline" @click="openIconPicker" class="select-icon-btn-sidebar">
            {{ t('admin.catalog.editor.information.icon.select') }}
          </v-btn>
        </div>
        <v-btn v-if="isCreationMode" block color="teal" variant="outlined" :disabled="!isFormValid || isSubmitting" class="mb-3" @click="createSection">{{ t('admin.catalog.editor.actions.create') }}</v-btn>
        <v-btn v-if="isEditMode" block color="teal" variant="outlined" :disabled="!isFormValid || isSubmitting" class="mb-3" @click="updateSection">{{ t('admin.catalog.editor.actions.save') }}</v-btn>
        <v-btn block color="grey" variant="outlined" class="mb-3" @click="catalogStore.closeSectionEditor()">{{ t('admin.catalog.editor.actions.cancel') }}</v-btn>
      </div>
    </div>
  </div>

  <v-dialog v-model="showOwnerSelector" max-width="700">
    <ItemSelector :title="t('admin.catalog.editor.information.owner.select')" search-service="searchUsers" action-service="returnSelectedUsername" :max-results="20" :max-items="1" :action-button-text="t('admin.catalog.editor.actions.save')" @close="showOwnerSelector = false" @action-performed="handleOwnerSelected" />
  </v-dialog>
  <v-dialog v-model="showBackupOwnerSelector" max-width="700">
    <ItemSelector :title="t('admin.catalog.editor.information.backupOwner.select')" search-service="searchUsers" action-service="returnSelectedUsername" :max-results="20" :max-items="1" :action-button-text="t('admin.catalog.editor.actions.save')" @close="showBackupOwnerSelector = false" @action-performed="handleBackupOwnerSelected" />
  </v-dialog>

  <IconPicker v-model="showIconPicker" :selected-icon="formData.icon_name" :selected-style="selectedIconStyle" :selected-size="selectedIconSize" @icon-selected="handleIconSelected" @style-changed="handleStyleChanged" @size-changed="handleSizeChanged" />
</template>
