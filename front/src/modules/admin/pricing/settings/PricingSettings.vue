...
const autoDeactivationEnabled = ref<boolean | null>(null)

// Define all settings that need to be loaded
const allSettings = [
  'pricelists.auto.deactivation.enabled'
]
...
async function loadSetting(settingName: string): Promise<boolean> {
  ...
  try {
    ...
    if (settingRetryAttempts.value[settingName] < 1) {
      ...
      uiStore.showErrorSnackbar(`error loading setting: ${settingName}`)
    }
...
function updateLocalSetting(settingName: string, value: any) {
  ...
  switch (settingName) {
    case 'pricelists.auto.deactivation.enabled':
      autoDeactivationEnabled.value = safeBoolean(value)
      break
  }
}
...
async function loadSettings() {
  ...
  try {
    console.log('Loading settings for Pricing.Settings')
    ...
    if (settings && settings.length > 0) {
      ...
      uiStore.showSuccessSnackbar(t('admin.pricing.settings.messages.loaded'))
    } else {
...
watch(
  ...
  (newValue) => {
    if (!isFirstLoad.value) {
      updateSetting('pricelists.auto.deactivation.enabled', newValue)
    }
  }
)
