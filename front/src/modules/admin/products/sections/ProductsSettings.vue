<!--
  File: ProductsSettings.vue - frontend file
  Description: Products settings section component for Products management module
  Purpose: Configure catalog products-related settings and parameters from Products module
  Version: 1.0.0
...
const section_path = 'AdminProducts';
...
async function loadSettings() {
  ...
  try {
    console.log('Loading settings for AdminProducts');
    ...
    console.error('Failed to load catalog products settings:', error);
...
watch(
  displayOptionsOnlyProducts, 
  (newValue) => {
    if (!isFirstLoad.value) {
      updateSetting('display.optionsOnlyProducts', newValue);
    }
  }
);
...
onMounted(() => {
  console.log('Products Settings component initialized');
  loadSettings();
});
</script>
...
<template>
  <div class="catalog-products-container">
    <h2 class="text-h6 mb-4">
      {{ t('admin.products.settings.title') }}
    </h2>
    ...
    <template v-if="!isLoadingSettings">
      ...
      <div class="settings-section">
        <div class="section-content">
          <!-- ==================== DISPLAY OPTIONS ONLY PRODUCTS SECTION ==================== -->
          <div class="d-flex align-center mb-2">
            <v-switch
              v-model="displayOptionsOnlyProducts"
              color="teal-darken-2"
              :label="t('admin.products.settings.displayOptionsOnly.label')"
              hide-details
              :disabled="isSettingDisabled('display.optionsOnlyProducts')"
              :loading="settingLoadingStates['display.optionsOnlyProducts']"
            />
            <v-tooltip
              v-if="settingErrorStates['display.optionsOnlyProducts']"
              location="top"
              max-width="300"
            >
              ...
              <div class="pa-2">
                <p class="text-subtitle-2 mb-2">
                  {{ t('admin.products.settings.messages.error.tooltip.title') }}
                </p>
                <p class="text-caption">
                  {{ t('admin.products.settings.messages.error.tooltip.retry') }}
                </p>
              </div>
            </v-tooltip>
          </div>
          ...
          <div class="color-picker-container mt-6">
            <v-text-field
              ...
              :label="t('admin.products.settings.cardColor.label')"
              variant="outlined"
              density="comfortable"
              placeholder="#E8F4F8"
              :rules="[v => /^#[0-9A-Fa-f]{6}$/.test(v) || t('admin.products.settings.cardColor.picker.validHex')]"
              color="teal"
            ...
            >
              <v-card>
                <v-card-title class="text-subtitle-1">
                  {{ t('admin.products.settings.cardColor.picker.title') }}
                </v-card-title>
                <v-card-text>
                  <div class="color-picker-content">
                    ...
                    <div class="preset-colors mb-4">
                      <div class="preset-title mb-2">
                        {{ t('admin.products.settings.cardColor.picker.basicColors') }}
                      </div>
                    ...
                    <div class="custom-color-input">
                      <div class="input-title mb-2">
                        {{ t('admin.products.settings.cardColor.picker.customColor') }}
                      </div>
                      <v-text-field
                        v-model="selectedColor"
                        :label="t('admin.products.settings.cardColor.picker.hexCode')"
                        variant="outlined"
                        density="compact"
                        placeholder="#000000"
                        :rules="[v => /^#[0-9A-Fa-f]{6}$/.test(v) || t('admin.products.settings.cardColor.picker.validHex')]"
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    ...
                    @click="applyColor"
                  >
                    {{ t('admin.products.editor.actions.save') }}
                  </v-btn>
                </v-card-actions>
              </v-card>
            </v-dialog>
