...
#
# version: 1.3.0
# purpose: copy new db migrations from db/ to deploy/; never overwrite existing files in deploy.
# file: prepare-deploy.sh (dev-ops/release/scripts)
# logic: for each file in db/migrations: if not in deploy -> copy; if in deploy and same content -> skip;
#        if in deploy and different content -> do not copy, warn with clear action hint; summary lists copied, unchanged, not updated.
# Demo data: single source of truth is db/demo-data; we do not copy it to deploy. User access to demo script — later, separately.
#
# changes in v1.3.0:
# - Removed copy of db/demo-data to deploy. Demo data source remains db/demo-data only; not synced to deploy.
# changes in v1.2.0:
